...
module.exports = function(db) {
	...
	async.series([
		function(next) {
			util.callbackify(() => db.client.collection('searchtopic').createIndex({content: 'text', uid: 1, cid: 1}, {background: true}))(next);
		},
		function(next) {
			util.callbackify(() => db.client.collection('searchtopic').createIndex({id: 1}, {background: true}))(next);
		},
		function(next) {
			util.callbackify(() => db.client.collection('searchpost').createIndex({content: 'text', uid: 1, cid: 1}, {background: true}))(next);
		},
		function(next) {
			util.callbackify(() => db.client.collection('searchpost').createIndex({id: 1}, {background: true}))(next);
		}
	], function(err) {
		if (err) {
			winston.error(err);
	...
	db.search = function(key, data, limit, callback) {
		...
		util.callbackify(() => db.client.collection('search' + key).aggregate(aggregate).toArray())(function(err, results) {
			if (err) {
				return callback(err);
			}
	...
	db.searchRemove = function(key, ids, callback) {
		...
		util.callbackify(() => db.client.collection('search' + key).deleteMany({id: {$in: ids}}))(function(err) {
			callback(err);
		});
	};
};
